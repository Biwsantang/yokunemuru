...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    searchName(
      variables?: SearchNameQueryVariables,
      requestHeaders?: Dom.RequestInit["headers"]
    ): Promise<SearchNameQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<SearchNameQuery>(SearchNameDocument, variables, {
            ...requestHeaders,
            ...wrappedRequestHeaders,
          }),
        "searchName",
    ...
    seasonal(variables?: SeasonalQueryVariables, requestHeaders?: Dom.RequestInit["headers"]): Promise<SeasonalQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<SeasonalQuery>(SeasonalDocument, variables, { ...requestHeaders, ...wrappedRequestHeaders }),
        "seasonal",
        "query"
      );
    },
